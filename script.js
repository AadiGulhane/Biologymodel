...
(function initMicroSim() {
  ...
  /* ── Build morpho butterfly lattice ─────────────────────── */
  (function buildMorpho() {
    const g = new THREE.Group();

    // Dark substrate base
    const baseM = new THREE.Mesh(new THREE.BoxGeometry(PW, 0.45, PD), matBase);
    baseM.position.set(MORPH_CX, -0.225, 0);
    baseM.renderOrder = 1;
    g.add(baseM);
    ...
    for (let si = 0; si < N_SP; si++) {
      ...
      const spineM = new THREE.Mesh(new THREE.BoxGeometry(SP_W, MH, SP_D), matSpine);
      spineM.position.set(sx, MH / 2, 0);
      spineM.renderOrder = 2;
      g.add(spineM);

      // Glowing cap at top of each spine
      const capM = new THREE.Mesh(new THREE.BoxGeometry(SP_W + 0.07, 0.09, SP_D + 0.05), matCap);
      capM.position.set(sx, MH + 0.045, 0);
      capM.renderOrder = 5;
      g.add(capM);
    }

    // Horizontal lamellae (shelves) between each spine pair
    for (let si = 0; si < N_SP - 1; si++) {
      const midX = (spineXs[si] + spineXs[si + 1]) / 2;
      const maxW = SP_STEP - SP_W - 0.05;
      for (let l = 0; l < N_RIB; l++) {
        const frac = l / (N_RIB - 1);
        const ribW = maxW * (1.0 - frac * 0.62);
        const ribD = SP_D * (1.0 - frac * 0.28);
        const lamM = new THREE.Mesh(new THREE.BoxGeometry(ribW, RIB_H, ribD), matLam);
        lamM.position.set(midX, ribYs[l], 0);
        lamM.renderOrder = 4;
        g.add(lamM);
      }
    }
  ...
  function buildFlatRays(flatOuts) {
    const segsOut = [];
    for (let ri = 0; ri < 6; ri++) {
      ...
      const hz   = (f - 0.5) * PD * 0.46; // slight Z spread for 3D depth
      const ex   = hx - rdx * (RT / cosA);
      const ez   = hz - rdx * 0.06;
      ...
      if (out === 'reflected') {
        ...
        const depth = 0.12 + ri * 0.10;
        raySeg.push({ p0: [hx, 0, hz], p1: [hx + rdx * 0.08, -Math.min(depth, 0.58), hz], col: RCOL.absorbed });
      } else {
        raySeg.push({ p0: [hx, 0, hz], p1: [hx + rdx * 0.16, SLAB_BOT, hz], col: RCOL.transmitted });
      }
      segsOut.push(...raySeg);
    }
    return segsOut;
  ...
  function buildMorphoRays(bflyOuts) {
    const segsOut = [];

    for (let ri = 0; ri < 8; ri++) {
      const f   = ri / 7;
      const ang = (22 + f * 28) * Math.PI / 180;
      const dx0 = Math.sin(ang), dy0 = -Math.cos(ang); // always downward on entry
      const out = bflyOuts[ri];
      const vi  = ri % (N_SP - 1);
      const lx  = spineXs[vi]     + SP_W * 0.5 + 0.024;
      const rx  = spineXs[vi + 1] - SP_W * 0.5 - 0.024;
      const hx  = (lx + rx) * 0.5 + (f - 0.5) * (rx - lx) * 0.46;
      const hz  = (f - 0.5) * PD * 0.42;
      const ex  = hx - dx0 * ((RT - MH) / Math.abs(dy0));
      const raySeg = [];

      // Segment 1 always: ray through open air to the top opening
      raySeg.push({ p0: [ex, RT, hz], p1: [hx, MH, hz], col: RCOL.incoming });

      // ── Helper: first wall or floor hit from (cx,cy) going (dx,dy) ──
      function entryWallHit(cx, cy, dx, dy) {
        let tE = Infinity, hit = 'floor';
        if (dx < 0 && cx > lx + 1e-5) { const t=(lx-cx)/dx; if(t>1e-5&&t<tE){tE=t;hit='L';} }
        if (dx > 0 && cx < rx - 1e-5) { const t=(rx-cx)/dx; if(t>1e-5&&t<tE){tE=t;hit='R';} }
        const tF = (0 - cy) / dy; // dy<0 → positive
        if (tF > 1e-5 && tF < tE) { tE = tF; hit = 'floor'; }
        return {
          t: tE, hit,
          ex: Math.max(lx+1e-4, Math.min(rx-1e-4, cx + dx*tE)),
          ey: Math.max(1e-4,     Math.min(MH-1e-4, cy + dy*tE))
        };
      }

      // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      //  REFLECTED: enter → hit topmost lamella → bounce out
      // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      if (out === 'reflected') {
        // Find the topmost rib the downward ray hits first
        let tRib = Infinity, ribHitY = MH;
        for (const ry of ribYs) {
          const rTop = ry + RIB_H * 0.5;
          if (rTop < MH - 1e-3) { // rib is inside the structure
            const t = (rTop - MH) / dy0; // MH→rTop going down: positive t
            if (t > 1e-5 && t < tRib) { tRib = t; ribHitY = rTop; }
          }
        }
        // Also check first wall — whichever is closer
        const wallT = entryWallHit(hx, MH, dx0, dy0);
        if (wallT.t < tRib) {
          // Wall is closer than any rib — bounce off wall then exit
          raySeg.push({ p0: [hx, MH, hz], p1: [wallT.ex, wallT.ey, hz], col: RCOL.incoming });
          // After wall bounce: dx flips, dy unchanged (still going down)
          const dx2 = -dx0;
          // Find next rib going down
          let t2 = Infinity, rib2Y = 0;
          for (const ry of ribYs) {
            const rTop = ry + RIB_H * 0.5;
            if (rTop < wallT.ey - 1e-5) {
              const t = (rTop - wallT.ey) / dy0;
              if (t > 1e-5 && t < t2) { t2 = t; rib2Y = rTop; }
            }
          }
          const ex2 = Math.max(lx, Math.min(rx, wallT.ex + dx2 * t2));
          raySeg.push({ p0: [wallT.ex, wallT.ey, hz], p1: [ex2, rib2Y, hz], col: bounceColor(0) });
          // Bounce off rib: dy flips upward
          const upT = (MH - rib2Y) / Math.abs(dy0);
          const exitX = Math.max(lx, Math.min(rx, ex2 + dx2 * upT));
          raySeg.push({ p0: [ex2, rib2Y, hz], p1: [exitX + dx2*0.3, MH + Math.abs(dy0)*2.6, hz], col: RCOL.reflected });
        } else {
          // Rib is the first hit — enter, hit rib, exit straight out
          const ribX = Math.max(lx, Math.min(rx, hx + dx0 * tRib));
          raySeg.push({ p0: [hx, MH, hz], p1: [ribX, ribHitY, hz], col: RCOL.incoming });
          // Reflect off rib top: dy goes positive (upward)
          const upT  = (MH - ribHitY) / Math.abs(dy0);
          const exitX = Math.max(lx, Math.min(rx, ribX + dx0 * upT));
          raySeg.push({ p0: [ribX, ribHitY, hz], p1: [exitX + dx0*0.3, MH + Math.abs(dy0)*2.6, hz], col: RCOL.reflected });
        }
        segsOut.push(...raySeg); continue;
      }

      // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      //  TRANSMITTED: enter → wall bounces → reach floor
      //  Ribs are bypassed (nanostructure allows partial transmission)
      // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      if (out === 'transmitted') {
        // Pre-trace: entry through open top, walls+floor only
        const e0 = entryWallHit(hx, MH, dx0, dy0);
        raySeg.push({ p0: [hx, MH, hz], p1: [e0.ex, e0.ey, hz], col: RCOL.incoming });
        if (e0.hit === 'floor') {
          raySeg.push({ p0: [e0.ex, 0, hz], p1: [e0.ex, -0.35, hz], col: RCOL.transmitted });
          segsOut.push(...raySeg); continue;
        }
        // Wall hit — bounce and trace to floor
        let cx = e0.ex, cy = e0.ey, dx = -dx0, dy = dy0;
        let reachedFloor = false;
        for (let b = 0; b < 6 && !reachedFloor; b++) {
          ...
          if (hitW === 'floor') {
            raySeg.push({ p0: [cx, cy, hz], p1: [nx, 0, hz], col: bounceColor(b) });
            raySeg.push({ p0: [nx, 0, hz], p1: [nx, -0.40, hz], col: RCOL.transmitted });
            reachedFloor = true;
          } else {
            raySeg.push({ p0: [cx, cy, hz], p1: [nx, ny, hz], col: bounceColor(b) });
            dx = -dx;
          }
          cx = nx; cy = ny;
        }
      ...
      // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      //  ABSORBED: deep wall entry → full rib+wall multi-bounce
      // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      {
        // Pre-trace to first wall (open top, ignore rib surfaces on entry)
        const e0 = entryWallHit(hx, MH, dx0, dy0);
        raySeg.push({ p0: [hx, MH, hz], p1: [e0.ex, e0.ey, hz], col: RCOL.incoming });

        if (e0.hit === 'floor') {
          raySeg.push({ p0: [e0.ex, 0, hz], p1: [e0.ex, -0.12, hz], col: RCOL.absorbed });
          segsOut.push(...raySeg); continue;
        }
        ...
        for (let b = 0; b < MAXB; b++) {
          ...
          // Lamella surfaces (both top and bottom faces, from correct direction)
          for (const ry of ribYs) {
            if (dy < 0 && cy > ry + RIB_H*0.5 + 1e-5) {
              const t=(ry+RIB_H*0.5-cy)/dy; if(t>1e-5&&t<tMin){tMin=t;hitW='rib';}
            } else if (dy > 0 && cy < ry - RIB_H*0.5 - 1e-5) {
              const t=(ry-RIB_H*0.5-cy)/dy; if(t>1e-5&&t<tMin){tMin=t;hitW='rib';}
            }
          }
          ...
          // Top exit — open boundary, ray escapes upward
          ...
          if (hitW === 'floor') {
            raySeg.push({ p0:[cx,cy,hz], p1:[nx,0,hz], col:RCOL.absorbed });
            cx=nx; cy=0; break;
          } else if (hitW === 'top') {
            // Escaped upward — show as partial reflection
            raySeg.push({ p0:[cx,cy,hz], p1:[nx+dx*0.3, MH+Math.abs(dy)*2.2, hz], col:RCOL.reflected });
            cx=nx; cy=ny; break;
          } else {
            raySeg.push({ p0:[cx,cy,hz], p1:[nx,ny,hz], col:bounceColor(Math.min(b,2)) });
            if (hitW==='L'||hitW==='R') dx=-dx; else dy=-dy;
            cx=nx; cy=ny;
          }

          if (b === MAXB - 1) {
            // Exhausted bounces — absorbed in-structure
            raySeg.push({ p0:[cx,cy,hz], p1:[cx, Math.max(0,cy-0.20), hz], col:RCOL.absorbed });
          }
        }
        segsOut.push(...raySeg);
      }
  ...
  function buildAllSegments(stats) {
    ...
    const flatOuts = Array.from({ length: 6 }, (_, i) =>
      i < nFR ? 'reflected' : i < nFR + nFA ? 'absorbed' : 'transmitted');

    const nBR = Math.round(8 * br);
    const nBA = Math.min(8 - nBR, Math.round(8 * ba));
    const bflyOuts = Array.from({ length: 8 }, (_, i) =>
      i < nBR ? 'reflected' : i < nBR + nBA ? 'absorbed' : 'transmitted');

    allSegments = [...buildFlatRays(flatOuts), ...buildMorphoRays(bflyOuts)];
  }
  ...
  function showAllRays() {
    segLines.forEach(sl => {
      // Set tip to p1 via the dynamic buffer
      sl.buf[3] = sl.p1.x; sl.buf[4] = sl.p1.y; sl.buf[5] = sl.p1.z;
      sl.attr.needsUpdate  = true;
      sl.line.visible      = true;
      sl.gLine.visible     = true;
  ...
  if (btnRun) btnRun.addEventListener('click', () => {
    if (mAnimState === 'idle' || mAnimState === 'done') {
      if (!allSegments.length) buildAllSegments(lastStats);
      createSegLines();
  ...
  function updateStatsBar(s) {
    ...
    el.innerHTML =
      `<span style="color:#00ff88">▌ Morpho: ${fmt(s.bflyAbs)} absorbed · ${fmt(s.bflyRefl)} reflected · ${fmt(s.bflyTrans)} transmitted</span>` +
