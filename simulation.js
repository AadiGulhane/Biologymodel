...
/* ─────────────────────────────────────────────────────────────────
   §6  DETERMINISTIC RAY PHYSICS  (BUG 2 + 3 FIXED)
   ─────────────────────────────────────────────────────────────────
   KEY FIX: space HIT positions evenly on panel, then backtrack to
   find entry point.  entX = hitX − rdx × (RAY_START_Y / cosθ).
   This guarantees all rays hit the panel regardless of sun angle.
*/
const BASE_ABS_FLAT = 0.68;
const MAX_BOUNCES   = 32;
const RAY_START_Y   = 9.5;
const MARGIN        = 0.30;  // inset from panel edge
...
function traceFlatRay(ix, nRays, sunDeg, intensity) {
  ...
  const rdy  = -cosT;
  const f    = nRays > 1 ? ix / (nRays - 1) : 0.5;

  // ── FIX: hit X evenly on panel surface ──
  const hitX = FLAT_CX - PW / 2 + MARGIN + f * (PW - 2 * MARGIN);
  const t    = cosT > 1e-6 ? RAY_START_Y / cosT : 1e9;
  const entX = hitX - rdx * t;           // backtrack to sky-level entry
  const entZ = -PD / 2 + 0.35 + f * (PD - 0.70);

  // Absorption: cos(θ) × BASE × intensity — deterministic by index
  const absProb = cosT * BASE_ABS_FLAT * intensity;
  const absorbed = (ix / nRays) < absProb;
  ...
  if (!absorbed) {
    // Specular: reflected direction = (rdx, +cosT) — angle of incidence = angle of reflection
    const rLen = 5.5;
    path.push(new THREE.Vector3(hitX + rdx * rLen, cosT * rLen, entZ));
  }

  return { path, absorbed, onPanel: true };
}
...
function traceButterflyRay(ix, nRays, sunDeg, intensity) {
  ...
  // ── FIX: hit X evenly on panel surface at ridge-top level ──
  const hitX  = BFLY_CX - PW / 2 + MARGIN + f * (PW - 2 * MARGIN);
  const tRidge = cosT > 1e-6 ? (RAY_START_Y - RH) / cosT : 1e9;
  const entX  = hitX - rdx0 * tRidge;    // backtrack to sky-level entry
  const entZ  = -PD / 2 + 0.35 + f * (PD - 0.70);
  ...
  // Determine if hitX falls in a valley or on a ridge wall
  const relX   = hitX - (BFLY_CX - PW / 2);
  const vi     = Math.floor(relX / RS);
  const vCenX  = BFLY_CX - PW / 2 + (vi + 0.5) * RS;
  const vL     = vCenX - RW / 2;   // valley inner left
  const vR     = vCenX + RW / 2;   // valley inner right

  const inValley = hitX >= vL - 1e-6 && hitX <= vR + 1e-6;

  if (!inValley) {
    // Hit ridge wall top → specular reflection upward (escaped = red)
    const rLen = 4.0;
    path.push(new THREE.Vector3(hitX + rdx0 * rLen, RH + cosT * rLen, entZ));
    return { path, absorbed: false, bounces: 0, hitWall: true };
  }

  // Inside valley: trace bouncing until floor (absorbed) or escape through top
  let cx = hitX, cy = RH;
  let dx = rdx0, dy = rdy0;
  let bounces  = 0;
  let absorbed = false;
  ...
  for (let b = 0; b < MAX_BOUNCES; b++) {
    const tL  = (dx < 0 && cx > vL + 1e-8) ? (vL - cx) / dx : Infinity;
    const tR  = (dx > 0 && cx < vR - 1e-8) ? (vR - cx) / dx : Infinity;
    ...
    if (Math.abs(tMn - tF) < 1e-7) {
      // Hit semiconductor floor → absorbed immediately
      absorbed = true;
      break;
    } else if (Math.abs(tMn - tT) < 1e-7 && dy > 0) {
      // Escaped through valley opening
      const rLen = 3.5;
      path.push(new THREE.Vector3(cx + dx * rLen, RH + cosT * rLen, entZ));
      escaped = true;
      break;
    } else {
      // Wall bounce: flip dx
    ...
    if (b >= MAX_BOUNCES - 1) { absorbed = true; }
  }
